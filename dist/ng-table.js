...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/filters/datepicker.html', '<span>{{params.filter()[name]}}</span> <input type="text", name="{{name}}", ng-disabled="$filterRow.disabled", ng-model="params.filter()[name]", class="input-filter form-control", placeholder="{{getFilterPlaceholderValue(filter, name)}}", data-date-format="yyyy-MM-dd", data-autoclose="1", data-container="body", bs-datepicker/> ');
